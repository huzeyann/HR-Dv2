...
from .transform import iden_partial, get_shift_transforms, get_flip_transforms, combine_transforms
...
def hires_dino(dino_name: DINONameOptions = "dino_vitb8", 
    ...
    model = HighResDINO(dino_name, stride, dtype, track_grad)

    fwd_shift, inv_shift = get_shift_transforms(shift_dists, 'Moore')
    if flip_transforms:  # add flip transforms
        fwd_flip, inv_flip = get_flip_transforms()
        fwd, inv = combine_transforms(fwd_shift, fwd_flip, inv_shift, inv_flip)
    else:
...
def hires_dino_small() -> HighResDINO:
    return hires_dino(dino_name="dino_vits8", 
                    stride=6, 
                    shift_dists=[2],
